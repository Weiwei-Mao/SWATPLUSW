...
Input file for initial conditions; source slot hmet_soil.
...
- Source inventory slot(s): `in_init%hmet_soil = "hmet_hru.ini"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `hmet_hru.ini` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_init%hmet_soil` defaulting to `hmet_hru.ini`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/hmet_hru_aqu_read.f90`.
