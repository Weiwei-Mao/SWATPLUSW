---
title: pest_water.ini input file
...
- Source inventory slot(s): `in_init%pest_water = "pest_water.ini"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `pest_water.ini` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_init%pest_water` defaulting to `pest_water.ini`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/pest_cha_res_read.f90`.
