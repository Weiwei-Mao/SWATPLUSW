---
title: path_water.ini input file
...
Input file for initial conditions; source slot path_water.
...
- Source inventory slot(s): `in_init%path_water = "path_water.ini"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `path_water.ini` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_init%path_water` defaulting to `path_water.ini`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/path_cha_res_read.f90`.
