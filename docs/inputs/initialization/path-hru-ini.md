...
Input file for initial conditions; source slot path_soil.
...
- Source inventory slot(s): `in_init%path_soil = "path_hru.ini"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `path_hru.ini` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_init%path_soil` defaulting to `path_hru.ini`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/path_hru_aqu_read.f90`.
