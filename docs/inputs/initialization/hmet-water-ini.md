---
title: hmet_water.ini input file
...
Input file for initial conditions; source slot hmet_water.
...
- Source inventory slot(s): `in_init%hmet_water = "hmet_water.ini"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `hmet_water.ini` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_init%hmet_water` defaulting to `hmet_water.ini`.
