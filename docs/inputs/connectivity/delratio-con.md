...
Input file for connect; source slot delr_con.
...
- Source inventory slot(s): `in_con%delr_con = "delratio.con"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `delratio.con` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_con%delr_con` defaulting to `delratio.con`.
