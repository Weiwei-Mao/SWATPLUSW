...
Input file for connect; source slot chan_con.
...
- Source inventory slot(s): `in_con%chan_con = "channel.con"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `channel.con` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_con%chan_con` defaulting to `channel.con`.
