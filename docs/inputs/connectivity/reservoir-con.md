---
title: reservoir.con input file
...
Input file for connect; source slot res_con.
...
- Source inventory slot(s): `in_con%res_con = "reservoir.con"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `reservoir.con` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_con%res_con` defaulting to `reservoir.con`.
