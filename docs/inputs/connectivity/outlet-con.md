...
Input file for connect; source slot out_con.
...
- Source inventory slot(s): `in_con%out_con = "outlet.con"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `outlet.con` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_con%out_con` defaulting to `outlet.con`.
