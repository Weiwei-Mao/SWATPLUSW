...
Input file for connect; source slot hruez_con.
...
- Source inventory slot(s): `in_con%hruez_con = "hru-lte.con"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `hru-lte.con` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_con%hruez_con` defaulting to `hru-lte.con`.
