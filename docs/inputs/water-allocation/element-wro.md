...
Input file for water-rights; source slot element.
...
- Source inventory slot(s): `in_watrts%element = "element.wro"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `element.wro` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_watrts%element` defaulting to `element.wro`.
