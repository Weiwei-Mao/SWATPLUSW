---
title: shade_factor.shf input file
...
Input file for shade factor; source slot ssff_shf.
...
- Source inventory slot(s): `in_shf%ssff_shf = "shade_factor.shf"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `shade_factor.shf` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_shf%ssff_shf` defaulting to `shade_factor.shf`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/shade_factor_read.f90`.
