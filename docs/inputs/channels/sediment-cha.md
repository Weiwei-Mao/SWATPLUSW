...
Input file for channel; source slot sed.
...
- Source inventory slot(s): `in_cha%sed = "sediment.cha"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `sediment.cha` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cha%sed` defaulting to `sediment.cha`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read_sed.f90`.
