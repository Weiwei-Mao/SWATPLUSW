---
title: temperature.cha input file
...
Input file for channel; source slot temp.
...
- Source inventory slot(s): `in_cha%temp = "temperature.cha"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `temperature.cha` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cha%temp` defaulting to `temperature.cha`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read_temp.f90`.
