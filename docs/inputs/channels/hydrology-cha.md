---
title: hydrology.cha input file
...
Input file for channel; source slot hyd.
...
- Source inventory slot(s): `in_cha%hyd = "hydrology.cha"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `hydrology.cha` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cha%hyd` defaulting to `hydrology.cha`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read_hyd.f90`, `SWATPLUS/swatplus/src/sd_hydsed_read.f90`.
