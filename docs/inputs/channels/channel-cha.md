...
Input file for channel; source slot dat.
...
- Source inventory slot(s): `in_cha%dat = "channel.cha"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `channel.cha` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cha%dat` defaulting to `channel.cha`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read.f90`.
