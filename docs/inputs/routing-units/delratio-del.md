...
Input file for delivery ratio; source slot del_ratio.
...
- Source inventory slot(s): `in_delr%del_ratio = "delratio.del"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `delratio.del` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_delr%del_ratio` defaulting to `delratio.del`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dr_db_read.f90`.
