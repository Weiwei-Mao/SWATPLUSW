...
Input file for delivery ratio; source slot hmet.
...
- Source inventory slot(s): `in_delr%hmet = "dr_hmet.del"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `dr_hmet.del` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_delr%hmet` defaulting to `dr_hmet.del`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dr_read_hmet.f90`.
