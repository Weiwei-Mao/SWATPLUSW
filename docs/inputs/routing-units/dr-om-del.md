...
Input file for delivery ratio; source slot om.
...
- Source inventory slot(s): `in_delr%om = "dr_om.del"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `dr_om.del` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_delr%om` defaulting to `dr_om.del`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dr_read_om.f90`.
