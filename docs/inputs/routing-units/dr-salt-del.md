...
Input file for delivery ratio; source slot salt.
...
- Source inventory slot(s): `in_delr%salt = "dr_salt.del"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `dr_salt.del` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_delr%salt` defaulting to `dr_salt.del`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dr_read_salt.f90`.
