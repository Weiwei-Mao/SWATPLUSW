...
Input file for delivery ratio; source slot path.
...
- Source inventory slot(s): `in_delr%path = "dr_path.del"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `dr_path.del` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_delr%path` defaulting to `dr_path.del`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dr_path_read.f90`.
