...
- Source inventory slot(s): `in_ru%ru_dr = "rout_unit.dr"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `rout_unit.dr` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_ru%ru_dr` defaulting to `rout_unit.dr`.
