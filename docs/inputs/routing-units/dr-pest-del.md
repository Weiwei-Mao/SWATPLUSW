...
Input file for delivery ratio; source slot pest.
...
- Source inventory slot(s): `in_delr%pest = "dr_pest.del"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `dr_pest.del` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_delr%pest` defaulting to `dr_pest.del`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dr_read_pest.f90`.
