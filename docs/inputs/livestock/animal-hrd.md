...
Input file for herd; source slot animal.
...
- Source inventory slot(s): `in_herd%animal = "animal.hrd"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `animal.hrd` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_herd%animal` defaulting to `animal.hrd`.
...
Herd, animal, and ranch routines when the livestock module is enabled.
