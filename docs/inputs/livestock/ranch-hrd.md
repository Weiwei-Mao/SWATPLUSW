...
Input file for herd; source slot ranch.
...
- Source inventory slot(s): `in_herd%ranch = "ranch.hrd"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ranch.hrd` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_herd%ranch` defaulting to `ranch.hrd`.
