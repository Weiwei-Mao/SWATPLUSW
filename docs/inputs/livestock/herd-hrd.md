...
Input file for herd; source slot herd.
...
- Source inventory slot(s): `in_herd%herd = "herd.hrd"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `herd.hrd` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_herd%herd` defaulting to `herd.hrd`.
