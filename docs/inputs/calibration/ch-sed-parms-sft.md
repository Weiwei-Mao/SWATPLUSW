---
title: ch_sed_parms.sft input file
...
Input file for calibration change; source slot ch_sed_parms_sft.
...
- Source inventory slot(s): `in_chg%ch_sed_parms_sft = "ch_sed_parms.sft"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ch_sed_parms.sft` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_chg%ch_sed_parms_sft` defaulting to `ch_sed_parms.sft`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read_parms_cal.f90`.
