---
title: ch_sed_budget.sft input file
...
Input file for calibration change; source slot ch_sed_budget_sft.
...
- Source inventory slot(s): `in_chg%ch_sed_budget_sft = "ch_sed_budget.sft"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ch_sed_budget.sft` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_chg%ch_sed_budget_sft` defaulting to `ch_sed_budget.sft`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read_orders_cal.f90`.
