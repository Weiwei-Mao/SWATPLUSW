---
title: plant_gro.sft input file
...
Input file for calibration change; source slot plant_gro_sft.
...
- Source inventory slot(s): `in_chg%plant_gro_sft = "plant_gro.sft"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `plant_gro.sft` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_chg%plant_gro_sft` defaulting to `plant_gro.sft`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/pl_read_regions_cal.f90`.
