---
title: plant_parms.sft input file
...
Input file for calibration change; source slot plant_parms_sft.
...
- Source inventory slot(s): `in_chg%plant_parms_sft = "plant_parms.sft"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `plant_parms.sft` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_chg%plant_parms_sft` defaulting to `plant_parms.sft`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/pl_read_parms_cal.f90`.
