...
Input file for conditional; source slot dtbl_scen.
...
- Source inventory slot(s): `in_cond%dtbl_scen = "scen_lu.dtl"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `scen_lu.dtl` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cond%dtbl_scen` defaulting to `scen_lu.dtl`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dtbl_scen_read.f90`.
