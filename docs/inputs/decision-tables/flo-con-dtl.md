...
Input file for conditional; source slot dtbl_flo.
...
- Source inventory slot(s): `in_cond%dtbl_flo = "flo_con.dtl"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `flo_con.dtl` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cond%dtbl_flo` defaulting to `flo_con.dtl`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/dtbl_flocon_read.f90`.
