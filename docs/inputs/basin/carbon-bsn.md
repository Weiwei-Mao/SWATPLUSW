...
Optional dynamic carbon basin parameter file when carbon behavior is enabled.
...
- Source inventory slot(s): `in_basin%carbon_bsn = "carbon.bsn"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `carbon.bsn` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_basin%carbon_bsn` defaulting to `carbon.bsn`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/carbon_bsn_read.f90`.
