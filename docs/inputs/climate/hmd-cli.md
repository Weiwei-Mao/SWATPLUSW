...
Input file for climate; source slot hmd_cli.
...
- Source inventory slot(s): `in_cli%hmd_cli = "hmd.cli"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `hmd.cli` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cli%hmd_cli` defaulting to `hmd.cli`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/cli_hmeas.f90`.
