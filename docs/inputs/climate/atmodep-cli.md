...
- Source inventory slot(s): `in_cli%atmo_cli = "atmodep.cli"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `atmodep.cli` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cli%atmo_cli` defaulting to `atmodep.cli`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/cli_read_atmodep.f90`.
