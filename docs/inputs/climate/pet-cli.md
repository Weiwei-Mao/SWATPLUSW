...
- Source inventory slot(s): `in_cli%pet_cli = "pet.cli"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `pet.cli` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_cli%pet_cli` defaulting to `pet.cli`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/cli_petmeas.f90`.
