---
title: carbon_lyr.bsn input file
...
Companion per-layer file for dynamic carbon basin parameters; derived from the selected carbon basin filename.

## Role In SWAT+

This page is a source-aligned input-reference stub for `carbon_lyr.bsn`. It exists so `docs/inputs/` has a durable page for each known SWAT+ input file or input-file family.
...
- Source inventory: carbon_bsn_read.f90 companion file.
...
- [SWAT+ input to output path](../input-output.md)