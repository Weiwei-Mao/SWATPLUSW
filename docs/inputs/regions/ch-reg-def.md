...
Input file for regions; source slot def_cha_reg.
...
- Source inventory slot(s): `in_regs%def_cha_reg = "ch_reg.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ch_reg.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_cha_reg` defaulting to `ch_reg.def`.
