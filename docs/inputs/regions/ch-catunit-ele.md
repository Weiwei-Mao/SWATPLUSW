---
title: ch_catunit.ele input file
...
Input file for regions; source slot ele_cha.
...
- Source inventory slot(s): `in_regs%ele_cha = "ch_catunit.ele"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ch_catunit.ele` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%ele_cha` defaulting to `ch_catunit.ele`.
