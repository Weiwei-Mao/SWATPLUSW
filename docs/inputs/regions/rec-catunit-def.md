---
title: rec_catunit.def input file
...
Input file for regions; source slot def_psc.
...
- Source inventory slot(s): `in_regs%def_psc = "rec_catunit.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `rec_catunit.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_psc` defaulting to `rec_catunit.def`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/rec_read_elements.f90`.
