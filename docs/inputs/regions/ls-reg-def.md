...
Input file for regions; source slot def_reg.
...
- Source inventory slot(s): `in_regs%def_reg = "ls_reg.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ls_reg.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_reg` defaulting to `ls_reg.def`.
