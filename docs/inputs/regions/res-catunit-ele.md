...
- Source inventory slot(s): `in_regs%ele_res = "res_catunit.ele"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `res_catunit.ele` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%ele_res` defaulting to `res_catunit.ele`.
