---
title: aqu_catunit.def input file
...
Input file for regions; source slot def_aqu.
...
- Source inventory slot(s): `in_regs%def_aqu = "aqu_catunit.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `aqu_catunit.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_aqu` defaulting to `aqu_catunit.def`.
