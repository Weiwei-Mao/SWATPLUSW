...
Input file for regions; source slot cal_lcu.
...
- Source inventory slot(s): `in_regs%cal_lcu = "ls_cal.reg"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ls_cal.reg` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%cal_lcu` defaulting to `ls_cal.reg`.
