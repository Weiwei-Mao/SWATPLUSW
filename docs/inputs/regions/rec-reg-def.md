...
Input file for regions; source slot def_psc_reg.
...
- Source inventory slot(s): `in_regs%def_psc_reg = "rec_reg.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `rec_reg.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_psc_reg` defaulting to `rec_reg.def`.
