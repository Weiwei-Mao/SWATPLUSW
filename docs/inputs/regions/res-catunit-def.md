---
title: res_catunit.def input file
...
Input file for regions; source slot def_res.
...
- Source inventory slot(s): `in_regs%def_res = "res_catunit.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `res_catunit.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_res` defaulting to `res_catunit.def`.
