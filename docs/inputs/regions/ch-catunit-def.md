---
title: ch_catunit.def input file
...
Input file for regions; source slot def_cha.
...
- Source inventory slot(s): `in_regs%def_cha = "ch_catunit.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ch_catunit.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_cha` defaulting to `ch_catunit.def`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/ch_read_elements.f90`.
