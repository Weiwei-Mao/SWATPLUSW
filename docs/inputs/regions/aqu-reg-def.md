...
Input file for regions; source slot def_aqu_reg.
...
- Source inventory slot(s): `in_regs%def_aqu_reg = "aqu_reg.def"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `aqu_reg.def` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%def_aqu_reg` defaulting to `aqu_reg.def`.
