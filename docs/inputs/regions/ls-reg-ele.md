...
Input file for regions; source slot ele_reg.
...
- Source inventory slot(s): `in_regs%ele_reg = "ls_reg.ele"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `ls_reg.ele` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%ele_reg` defaulting to `ls_reg.ele`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/reg_read_elements.f90`.
