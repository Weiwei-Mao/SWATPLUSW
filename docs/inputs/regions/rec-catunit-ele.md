---
title: rec_catunit.ele input file
...
Input file for regions; source slot ele_psc.
...
- Source inventory slot(s): `in_regs%ele_psc = "rec_catunit.ele"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `rec_catunit.ele` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_regs%ele_psc` defaulting to `rec_catunit.ele`.
