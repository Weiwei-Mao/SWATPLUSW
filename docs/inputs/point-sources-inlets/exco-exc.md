...
Input file for exco (recall constant); source slot exco.
...
- Source inventory slot(s): `in_exco%exco = "exco.exc"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `exco.exc` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_exco%exco` defaulting to `exco.exc`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/exco_db_read.f90`.
