---
title: exco_path.exc input file
...
Input file for exco (recall constant); source slot path.
...
- Source inventory slot(s): `in_exco%path = "exco_path.exc"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `exco_path.exc` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_exco%path` defaulting to `exco_path.exc`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/exco_read_path.f90`.
