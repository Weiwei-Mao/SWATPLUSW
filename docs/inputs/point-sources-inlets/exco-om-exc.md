...
Input file for exco (recall constant); source slot om.
...
- Source inventory slot(s): `in_exco%om = "exco_om.exc"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `exco_om.exc` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_exco%om` defaulting to `exco_om.exc`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/exco_read_om.f90`.
