---
title: exco_salt.exc input file
...
Input file for exco (recall constant); source slot salt.
...
- Source inventory slot(s): `in_exco%salt = "exco_salt.exc"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `exco_salt.exc` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_exco%salt` defaulting to `exco_salt.exc`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/exco_read_salt.f90`.
