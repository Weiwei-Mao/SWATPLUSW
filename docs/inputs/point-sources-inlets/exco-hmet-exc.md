---
title: exco_hmet.exc input file
...
Input file for exco (recall constant); source slot hmet.
...
- Source inventory slot(s): `in_exco%hmet = "exco_hmet.exc"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `exco_hmet.exc` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_exco%hmet` defaulting to `exco_hmet.exc`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/exco_read_hmet.f90`.
