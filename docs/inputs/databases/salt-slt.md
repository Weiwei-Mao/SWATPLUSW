...
Input file for HRU databases; source slot saltcom_db.
...
- Source inventory slot(s): `in_parmdb%saltcom_db = "salt.slt"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `salt.slt` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_parmdb%saltcom_db` defaulting to `salt.slt`.
