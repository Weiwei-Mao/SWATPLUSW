---
title: pathogens.pth input file
...
Input file for HRU databases; source slot pathcom_db.
...
- Source inventory slot(s): `in_parmdb%pathcom_db = "pathogens.pth"`.
...
Local demo evidence: the `Osu_1hru` scenario does not include a concrete `pathogens.pth` file, or the file family is inactive there. Use another scenario or the reader routine for field-level confirmation.
...
- Source slot: `in_parmdb%pathcom_db` defaulting to `pathogens.pth`.
- Candidate reader/source files: `SWATPLUS/swatplus/src/path_parm_read.f90`.
