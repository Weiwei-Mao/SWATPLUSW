...
> [!info] Input File
> Declared in `file.cio` field `om_water`. See [[file.cio]] for the controlling file map.

## Overview
- **Declared in `file.cio` field**: `om_water`
...
| `om_init_water%flo`  | `real`                                         | ==-==       | ==Fraction of bankfull channel depth== | [[hydrograph_module.f90#om_init_water]] | [[om_water_init.f90]]:54 |
